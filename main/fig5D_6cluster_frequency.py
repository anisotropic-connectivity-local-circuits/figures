...
ax.errorbar(x, tuned_means, yerr=tuned_SEM,
            color=color['tuned'], label="tuned aniso.")

# ax.errorbar(x, aniso_means, color=color['aniso'], yerr=aniso_SEM)
# ax.errorbar(x, aniso_rew_means, color=color['rew'], yerr=aniso_rew_SEM)
#ax.errorbar(x, tuned_dist_means, color=col.dist, yerr=tuned_dist_SEM)


ax.set_xlim(0,max_count+0.5)
ax.set_yscale('log')

ax.set_xlabel('connections in clusters of 6 Cells', labelpad=18, size=15)
