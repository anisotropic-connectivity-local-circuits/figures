...
with open(dpath+'nmotif_ecounts_dist_n3_S2500K.p', 'rb') as pfile:
    dist_data = pickle.load(pfile)
with open(dpath+'nmotif_ecounts_rew-dist_n3_S2500K.p', 'rb') as pfile:
...
ax.errorbar(x[:-1], dist_rew_means[:-1], yerr=dist_rew_SEM[:-1],
            color='k', label="rewired")
ax.errorbar(x[:-1], dist_means[:-1], yerr=dist_SEM[:-1],
