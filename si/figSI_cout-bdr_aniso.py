...
for gid in range(ngraphs):
    if plot:
        ...
        in_aniso_all[gid,:]+=np.histogram(out_nb[cn==2], bins,
                                          density=True)[0]
        ...
        g = gt.load_graph(gpath)
        pairs, cn, in_nb, out_nb = get_common_neighbours(g)
        in_r025aniso_all[gid,:]+=np.histogram(out_nb[cn==2], bins,
                                              density=True)[0]
        ...
        g = gt.load_graph(gpath)
        pairs, cn, in_nb, out_nb = get_common_neighbours(g)
        in_raniso_all[gid,:]+=np.histogram(out_nb[cn==2], bins,
                                           density=True)[0]
